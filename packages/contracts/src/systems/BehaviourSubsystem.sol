...
contract BehaviourSubsystem is System {
  ...
  function movementTicks(bytes32 causedBy, bytes32 entity, PositionData memory pos) public {
    ...
    if (NPC.get(entity) > uint32(NPCType.None) && Rules.onMap(pos.x, pos.y)) {
      ...
      for (uint i = 0; i < positions.length; i++) {
        ...
        if (npcType > NPCType.None) {
          tickBehaviour(causedBy, entities[0], entity, positions[i], pos);
        }
    ...
    for (uint i = 0; i < globalEntities.length; i++) {
      if (Rules.hasTicked(globalEntities[i])) {
        continue;
      }
  ...
  function tickAction(bytes32 causedBy, bytes32 entity, PositionData memory entityPos) public {
    ...
    if (Wander.get(entity) > 0) {
      doWander(causedBy, entity, entityPos);
    }


    uint32 fling = Fling.get(entity);
    if (fling > 0) {
      PositionData memory isNewPos = Position.get(entity);
      if(entityPos.x == isNewPos.x && entityPos.y == isNewPos.y) {
        aoeFling(causedBy, entity, entityPos);
      }
    }
  ...
  function tickBehaviour(
    ...
    uint32 fling = Fling.get(entity);
    if (fling == distance) {
      callFling(causedBy, entity, target, entityPos, targetPos);
    }
    uint32 seek = Seek.get(entity);
    if (seek == distance) {
      doSeek(causedBy, entity, target, entityPos, targetPos);
    }
    uint32 aggro = Aggro.get(entity);
    if (aggro > 0 && aggro == distance) {
      doAggro(causedBy, entity, target, entityPos, targetPos);
    }
    uint32 archer = Archer.get(entity);
    if (archer > 0 && archer >= distance) {
      doArrow(causedBy, entity, target, entityPos, targetPos);
    }
    uint32 cursed = Cursed.get(entity);
    if (cursed > 0 && cursed >= distance) {
      doCurse(causedBy, entity, target, entityPos, targetPos);
    }
    uint32 thief = Thief.get(entity);
    if (thief > 0 && thief >= distance) {
      doThief(causedBy, entity, target, entityPos, targetPos);
    }
  ...
  function aoeFling(
    ...
    for (uint i = 0; i < positions.length; i++) {
      ...
      callFling(causedBy, entity, entities[0], entityPos, positions[i]);
    }
  }
  ...
  function doAggro(
    ...
    if (canAggroEntity(entity, target) == false) {
      return;
    }
